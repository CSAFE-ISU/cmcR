...
km1_unknown1_cmcs$initialCMCs[[1]][[1]]  %>%
  dplyr::arrange(cellNum)

km1_unknown2_cmcs$initialCMCs[[1]][[1]]  %>%
  dplyr::arrange(cellNum)
```

```{r}
blankImage <- matrix(NA,
                     nrow = nrow(km1_Residuals$x3p$surface.matrix),
                     ncol = ncol(km1_Residuals$x3p$surface.matrix))

cmcR::getHighCCFPairs(km1_Residuals$x3p,
                      unknown1_Residuals$x3p,
                      cellCCF_output = km1_unknown1_comparison$comparison_1to2)

for(ind in 1:length(im1_splitShifted)){
  cellRowInd <- cellLocs[[ind]][1]:cellLocs[[ind]][2]
  cellColInd <- cellLocs[[ind]][3]:cellLocs[[ind]][4]
  blankImage[cellRowInd,cellColInd] <- as.matrix(im1_splitShifted[[ind]])
}
...
imager::imlist(
  "fadul1" = blankImage %>%
    t() %>%
    imager::as.cimg() %>%
    imager::imrotate(angle = -90),
  "fadul2" = congruentMatchingCells:::rotateSurfaceMatrix(fadul2_cropped,
                                                          theta = -24) %>%
    t() %>%
    imager::as.cimg() %>%
    imager::imrotate(angle = -90)
) %>%
  plot(xlim = c(0,max(nrow(fadul1_cropped),
                      nrow(fadul2_cropped))),
       ylim = c(0,max(ncol(fadul1_cropped),
                      ncol(fadul2_cropped))),
       layout = "row")
```
