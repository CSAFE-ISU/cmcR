...
km2_residuals$surface.matrix <- km2_residuals$surface.matrix %>%
  cmcR::preProcess_ransac() %>%
  cmcR::preProcess_levelBF(useResiduals = TRUE) %>%
  cmcR::preProcess_cropWS()
```
...
```{r}
houghCircle <- cmcR::preProcess_detectFPCircle(km1_residuals$surface.matrix)

km1_residuals$surface.matrix <- cmcR::preProcess_removeFPCircle(surfaceMat = km1_residuals$surface.matrix,
                                                                fpImpressionCircle = houghCircle)
```
