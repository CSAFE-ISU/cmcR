...
```{r}
km1_raw <- x3ptools::read_x3p("https://tsapps.nist.gov/NRBTD/Studies/CartridgeMeasurement/Details/2d9cc51f-6f66-40a0-973a-a9292dbee36d")
km2_raw <- x3ptools::read_x3p("https://tsapps.nist.gov/NRBTD/Studies/CartridgeMeasurement/Details/cb296c98-39f5-46eb-abff-320a2f5568e8")
unknown1_raw <- x3ptools::read_x3p("https://tsapps.nist.gov/NRBTD/Studies/CartridgeMeasurement/Details/8ae0b86d-210a-41fd-ad75-8212f9522f96")
unknown2_raw <- x3ptools::read_x3p("https://tsapps.nist.gov/NRBTD/Studies/CartridgeMeasurement/Details/9a054edf-f648-40a5-a061-b05ad036c61e")
```
...
km1_residuals$surface.matrix <- km1_residuals$surface.matrix %>%
  preProcess_ransac(inlierThreshold = 1e5,
                    finalSelectionThreshold = 2*(1e5),
                    iters = 150) %>%
  preProcess_levelBF(useResiduals = TRUE) %>%
  preProcess_ransac(inlierThreshold = .1*(1e5),
                    finalSelectionThreshold = 2*(1e5),
                    iters = 300) %>%
  preProcess_levelBF(useResiduals = TRUE) %>%
...
The improved CMC method did not yield any "high CMCs" for the comparison between km1 and unknown1. As such, we would assign the pair whichever was the minimum number of initial CMCs calculated between both directions. The CMC(s) between km1 and unknown1 are shown below.

```{r eval=FALSE}
km1_unknown1_cmcs$initialCMCs[[which.min(c(nrow(km1_unknown1_cmcs$initialCMCs[[1]]),
                                                nrow(km1_unknown1_cmcs$initialCMCs[[2]])))]]
```
...
Interestingly, we see that the comparison between km2 and unknown1 *does* actually yield high CMCs, although only 5. This illustates an important property of this method - its extreme sensitivity to making the "correct" pre, inter, and post-processing decisions. For example, changing `ccf_thresh` from .8 to .81 in the call to `cmcFilter_improved` between km1 and unknown1 above yields 0 initial and high CMCs.  
...
```{r eval=FALSE}
cmcR::cmcPlot(x3p1 = km1_residuals$x3p,
...
In contrast, the high CMCs between km1 and unknown2 are shown below.
...
The same types of plots can be made for the comparison between km2 and the unknown source scans. Below are the high CMCs between km2 and unknown1.

```{r eval=FALSE}
cmcR::cmcPlot(x3p1 = km2_residuals$x3p,
              x3p2 = unknown1_residuals$x3p,
