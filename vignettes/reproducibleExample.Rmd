...
x3p1 <- x3ptools::read_x3p("path/to/file") %>%
...
x3p1$surface.matrix <- x3p1$surface.matrix %>%
  ...
  preProcess_gaussFilter(res = x3p$header.info$incrementY,#resolution of downsampled scan
                         wavelength = c(16,250),
                         filtertype = "bp")
```
...
```{r}
dat <- cellCCF(km1_Residuals$x3p,km2_Residuals$x3p,
...
dat$ccfResults %>% topResultsPerCell()
```
