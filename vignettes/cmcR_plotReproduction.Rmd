...
```{r include=FALSE,eval=FALSE}
#performing log/sigmoidal transformations don't seem to affect scale -- need
...
fadul1.1$x3p %>%
  ...
  #                                        na.rm = TRUE),
  #                      na.value = "grey80") +
  scale_fill_distiller(na.value = "grey80",palette = "PuOr") +
  theme_grey() +
  coord_fixed(expand = FALSE)
...
fadul1.2$x3p %>%
  ...
  #                      na.value = "grey80") +
  # scale_fill_distiller(na.value = "grey80",palette = "PuOr",
  #                      breaks = function(lims) {quantile(as.vector(fadul1.1$x3p$surface.matrix),c(0,.2,.4,.5,.6,.8,1),na.rm = TRUE)},
  #                      ) +
...
TODO:
- Shift by median, not mean
- Add to legend percentile (microns in parentheses)
- Frame legend in black
- Index cells by (1,1), (1,2),...,(1,7),(2,1),...,(7,7)
- Change to 8x8 cells to reflect Tong et al.
...
TODO: 
- Change labels in cells to start at top-left corner and count right/down
- If all cells were identified as final CMCs, then need to ensure that they are all colored black -- currently all colored red. Maybe make cmc a factor and choose levels.
...
# gridExtra::grid.arrange(kmPlts$finalCMC)

kmPlts$finalCMC$grobs[[1]] %>%
  plot()

kmPlts$finalCMC$grobs[[2]] %>%
  plot()
```
...
knmPlts$initialCMC$grobs[[1]] %>%
  plot()

knmPlts$initialCMC$grobs[[2]] %>%
  plot()
```
...
ccfMapPlot(mat1 = kmCellRegionPairs$`21`$`x = 1 - 83,y = 414 - 496`[[1]],
           mat2 = kmCellRegionPairs$`21`$`x = 1 - 83,y = 414 - 496`[[2]],
           type = "raster")
